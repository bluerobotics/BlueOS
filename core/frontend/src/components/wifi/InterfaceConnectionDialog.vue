  ...
  >
    <v-card>
      ...
      <v-card-text v-if="connecting">
        Connecting on {{ interfaceName }}, please wait...
        <v-progress-linear
          indeterminate
          color="primary"
          class="mb-0"
...
import { ConnectRequest, Network, NetworkCredentials } from '@/types/wifi'
...
export default Vue.extend({
  name: 'InterfaceConnectionDialog',
  components: {
    PasswordInput,
  },
  model: {
  ...
  methods: {
    ...
    async connectToWifiNetwork(): Promise<void> {
      ...
      try {
        ...
        await back_axios({
          method: 'post',
          url: `${wifi.API_URL_V2}/wifi/connect`,
          timeout: 50000,
          data: request,
        })
    ...
    async removeSavedWifiNetwork(): Promise<void> {
      ...
      await back_axios({
        method: 'delete',
        url: `${wifi.API_URL_V2}/wifi/saved/${encodeURIComponent(this.network.ssid)}`,
        timeout: 10000,
      }).then(() => {
