...
export default Vue.extend({
  ...
  computed: {
    ...
    is_hotspot_mode(): boolean {
      return this.interface_data?.mode === WifiInterfaceMode.HOTSPOT
        || this.interface_data?.mode === WifiInterfaceMode.DUAL
    },
    wifi_icon(): string {
      if (!this.interface_data) {
        return 'mdi-wifi-sync'
      }
      if (this.is_hotspot_mode) {
        if (this.interface_data.mode === WifiInterfaceMode.DUAL && this.interface_data.connected) {
