...
export default Vue.extend({
  ...
  computed: {
    ...
    this_servo_gpio(): number | undefined {
      ...
      return servoNumberToGpio(this.servo_number, this.board_name)
    },
  ...
  methods: {
    ...
    onChange(value: {text: string, value: Parameter}) {
      ...
      // Capture previous owner before making any changes (computed updates after setParam)
      const previousPinParam = this.function_pin_parameter_using_this_gpio
      if (is_relay) {
        ...
        if (previousPinParam && previousPinParam.name !== pin_param_name) {
          mavlink2rest.setParam(previousPinParam.name, -1, autopilot_data.system_id, 'MAV_PARAM_TYPE_INT8')
        }
      }
      if (is_leak) {
        const paramName = value.value.name
