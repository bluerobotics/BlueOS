...
export default Vue.extend({
  ...
  computed: {
    actuator_number(): number | undefined {
      ...
      return parseInt(actuator_number)
    },
    ...
    this_actuator_functions(): number[] {
        ...
        return Object.entries(this.options).filter(([key, value]) => value.startsWith(`actuator_${this.actuator_number}_`)).map(([key, value]) => parseInt(key))
    },
