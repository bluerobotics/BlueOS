  ...
  >
    ...
    <v-card class="pa-5" style="overflow-x: hidden;">
      ...
      <v-sheet v-if="display_status" class="elevation-1 pa-3 ma-8">
        <StatusTextWatcher :filter="/thrust|Thrust|Motor/" @message="updateStatus" />
      </v-sheet>
      <v-row v-if="status == statuses.IDLE">
        <v-col rows="3" class="justify-center">
          <ThemedSVG src="/public/img/icons/motordetection.svg" />
        </v-col>
        <v-col cols="8">
          <v-card-text>
            <p>
            ...
            <p>
              The test relies on the motors being correctly connected according to the selected vehicle frame, and
              spins each motor individually to check whether it generates thrust in the expected direction.
              The vehicle should be in calm water, and the autopilot must be in manual mode to be able to run the test.
            </p>
          </v-card-text>
        </v-col>
      </v-row>
      ...
      <v-card-text v-if="status == statuses.FAILED" class="ma-8">
        <p>
          Motor testing can fail due to any unexpected motions of the vehicle. Here are some common failure causes,
...
export default Vue.extend({
  ...
  watch: {
    ...
    display_status(new_state: status) {
      console.log('display status:', new_state)
    },
  },
  methods: {
    async start() {
