...
import { fetchCurrentBoard } from '../autopilot/AutopilotManagerUpdater'
...
export default {
  ...
  watch: {
    ...
    componentModel() {
      console.log('Component model changed, reloading board model...')
  ...
  methods: {
    rotateObject(rotation: Rotation | undefined) {
      ...
      if (rotation.name.startsWith('UNSUPPORTED')) {
        console.log(`Not visualizing unsupported rotation (${rotation.rotationNumber})`)
        this.object.visible = false
        return
      }

      this.object.visible = true
      console.log(`Applying rotation: roll=${rotation.roll}, pitch=${rotation.pitch}, yaw=${rotation.yaw}`)
    ...
    async add_board_model() {
      if (this.scene) {
        try {
          ...
          console.log('Loading board model from path:', modelPath)
