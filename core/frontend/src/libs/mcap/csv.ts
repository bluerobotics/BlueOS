...
import { Mp4ExportRange } from './export'
import {
  McapChannel, McapIndexedReader, McapMessage,
} from './reader'

export type { McapRecordingChannel } from './channels'
export { defaultSelectedChannelIds, isOpaqueByteChannel, listMcapChannels } from './channels'
...
async function collectRows(
  ...
  function report(logTime: bigint): void {
    const now = Date.now()
    if (!onProgress || now - lastProgressAt < 200) {
      return
    }
    lastProgressAt = now
    onProgress({
