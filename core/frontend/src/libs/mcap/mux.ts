 ...
 * this file does not.
 *
 * Decoder codec strings and coded size come from mediabunny's Annex B parsers (not re-exported from
 * the package root; they live next to the muxer). The first EncodedVideoPacketSource.add still
 * needs those values, and a probe-mux round trip would write the placeholder size into tkhd/stsd.
 */
...
async function muxFrames(
  ...
  const output = new Output({
    format: new Mp4OutputFormat({
      ...
      minimumFragmentDuration: fragmented ? 0 : undefined,
    }),
...
export async function muxFragmentedMp4(frames: AnnexBFrame[], info: VideoDecoderInfo): Promise<Uint8Array> {
  return muxFrames(frames, info, true)
}

/** Ordinary MP4 with the moov at the end, which players and editors that do not speak fMP4 still read. */
export async function muxFileMp4(
  frames: AnnexBFrame[],
  info: VideoDecoderInfo,
  onBytes?: (bytes: number) => void,
): Promise<Blob> {
  const data = await muxFrames(frames, info, false, onBytes)
  return new Blob([data], { type: 'video/mp4' })
}

/** Incremental MP4 writer. `fragmented` is for MSE; otherwise the moov is written at the end. */
class Mp4OutputStream {
  ...
  private constructor(
    ...
    this.output = new Output({
      format: new Mp4OutputFormat({
        fastStart: fragmented ? 'fragmented' : false,
        minimumFragmentDuration: fragmented ? fragmentSeconds : undefined,
      }),
      target: new AppendOnlyStreamTarget(new WritableStream({
  ...
  static async open(
    info: VideoDecoderInfo,
    onData: (data: Uint8Array) => Promise<void> | void,
    options: { fragmented: boolean, fragmentSeconds?: number },
  ): Promise<Mp4OutputStream> {
    const stream = new Mp4OutputStream(info, onData, options.fragmented, options.fragmentSeconds ?? 0.5)
...
export class Mp4MediaStream {
  private constructor(private inner: Mp4OutputStream) {}
  ...
  static async open(
    ...
    const inner = await Mp4OutputStream.open(info, onData, { fragmented: true, fragmentSeconds })
    return new Mp4MediaStream(inner)
  }

  add(frame: AnnexBFrame): Promise<void> {
    return this.inner.add(frame)
  }
...
export class Mp4FileStream {
  ...
  static async open(
    ...
    const inner = await Mp4OutputStream.open(info, onData, { fragmented: false })
    return new Mp4FileStream(inner)
  }
