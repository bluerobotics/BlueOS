...
export class HttpByteSource implements ByteSource {
  ...
  private async request(range: string, signal?: AbortSignal): Promise<{ data: Uint8Array, total: number }> {
    // Chrome holds an HTTP-cache lock per URL, so sequential Range reads of one recording stall
    // behind each other (Firefox does not). A distinct query string gives each read its own lock;
    // nginx still serves the same file.
    const separator = this.url.includes('?') ? '&' : '?'
    const url = `${this.url}${separator}range=${encodeURIComponent(range)}`
    const response = await fetch(url, { cache: 'no-store', headers: { Range: range }, signal })
