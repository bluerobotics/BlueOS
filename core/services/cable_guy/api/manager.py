...
from commonwealth.utils.DHCPDiscovery import DHCPDiscoveryError, has_dhcp_server
...
class EthernetManager:
    ...
    async def dhcp_server_found_on_network(self, interface_name: str) -> bool:
        ...
        """
        try:
            return bool(await has_dhcp_server(interface_name))
    ...
    async def set_configuration(self, interface: NetworkInterface, watchdog_call: bool = False) -> None:
        ...
        for address in interface.addresses:
            if address.mode == AddressMode.Unmanaged:
                ...
                if await self.dhcp_server_found_on_network(interface.name):
                    logger.info("DHCP server already found on network. Skipping backup server.")
                    continue
