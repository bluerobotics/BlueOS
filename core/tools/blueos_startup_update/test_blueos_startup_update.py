...
sys.path.append(str(REPOSITORY_PATH / "core" / "libs" / "commonwealth" / "src"))
...
def test_merging_board_sections_keeps_the_order_the_firmware_applies() -> None:
    # A duplicate header reopens the same filter, so the firmware already reads the strays' lines
    # after the survivor's. Hoisting them above it would flip the effective value of any key the user
    # set under both headers, and two headers is what one boot on the bad release leaves behind
    config_content = ["[pi4]", "arm_freq=1500", "", "[pi4]", "arm_freq=1800"]

    blueos_startup_update.boot_config_merge_duplicated_sections(config_content, "pi4")

    headers = config_content.count("[pi4]")
    assert headers == 1, f"a device with two stray headers kept {headers} of them, the merge skipped the pair"
    settings = [line for line in config_content if line.startswith("arm_freq=")]
    assert settings == ["arm_freq=1500", "arm_freq=1800"], (
        f"the merge left the user configuration as {settings}: the firmware applies the last line of a "
        "filter, so the effective value changed while repairing the file"
    )
    (start, end) = blueos_startup_update.boot_config_get_or_append_section(config_content, "pi4")
    assert set(settings) <= set(config_content[start:end]), (
        f"the merged configuration landed outside [pi4] at {config_content[start:end]}, so the patches "
        "would not see it and would append their own copy on every boot"
    )
...
def fake_host_run_command(host_files: Dict[str, str]) -> Callable[..., "subprocess.CompletedProcess[str]"]:
    """A run_command that answers the patcher's own probe from a fake host, so the guard is exercised
    as written instead of against a stubbed return value."""

    def run_command(command: str, _check: bool = True, _log_output: bool = True) -> "subprocess.CompletedProcess[str]":
        match = re.fullmatch(r'head -n 1 "(?P<path>[^"]+)"', command)
        assert match, f"the guard ran an unexpected command on the host: {command}"
        lines = host_files.get(match["path"], "").splitlines()
        return subprocess.CompletedProcess(command, 0, stdout=f"{lines[0]}\n" if lines else "", stderr="")

    return run_command


def locate_boot_file_on_host(host_files: Dict[str, str], candidates: List[str]) -> Optional[str]:
    located = next((path for path in candidates if path in host_files), None)
    with patch.object(blueos_startup_update, "locate_file", lambda _candidates: located):
        with patch.object(blueos_startup_update, "run_command", fake_host_run_command(host_files)):
            return blueos_startup_update.locate_boot_file(candidates)


@pytest.mark.parametrize("boot_file", ["config.txt", "cmdline.txt"])
def test_startup_patcher_refuses_the_bookworm_boot_stub(boot_file: str) -> None:
    # When /boot/firmware is not mounted on a Bookworm host, find(1) settles on the /boot stub. The
    # install scripts refuse it by its marker; the patcher has to as well, or it writes the board
    # section into a file the firmware never reads, reboots, and finds its own work in place forever.
    candidates = [f"/boot/firmware/{boot_file}", f"/boot/{boot_file}"]
    at_boot = f"/boot/{boot_file}"

    assert locate_boot_file_on_host({at_boot: BOOT_STUBS[boot_file]}, candidates) is None

    # A real config.txt at /boot is a Bullseye boot partition, not a stub, and has to be accepted
    assert locate_boot_file_on_host({at_boot: BULLSEYE_STOCK_CONFIG_TXT}, candidates) == at_boot

    # The marker in a comment does not make a mounted boot partition a stub. Refusing it would leave
    # the board with no overlays, no cgroups and no OTG ethernet while main() still reported success,
    # which is the very bug this guard exists to prevent
    managed = f"# DO NOT EDIT THIS FILE - it is managed by provisioning\n{BULLSEYE_STOCK_CONFIG_TXT}"
    assert locate_boot_file_on_host({at_boot: managed}, candidates) == at_boot


def test_startup_patcher_resolves_both_boot_files_through_the_stub_guard() -> None:
    # The guard only protects the board if main() resolves through it. Reverting these two call sites
    # to locate_file reproduces the original bug verbatim, so pin them by driving main() itself.
    host_files = {f"/boot/{name}": stub for name, stub in BOOT_STUBS.items()}

    with patch.object(blueos_startup_update, "locate_file", lambda candidates: candidates[-1]), patch.object(
        blueos_startup_update, "run_command", fake_host_run_command(host_files)
    ), patch.object(blueos_startup_update, "run_command_is_working", lambda: False), patch.object(
        blueos_startup_update, "BOOT_LOOP_DETECTOR", "/tmp/a-boot-loop-detector-that-is-not-there"
    ), patch.object(
        blueos_startup_update, "config_file", None
    ), patch.object(
        blueos_startup_update, "cmdline_file", None
    ), patch.dict(
        os.environ, {"GIT_DESCRIBE_TAGS": "1.4.0-1-gabcdef0"}
    ):
        assert blueos_startup_update.main() == 0

        resolved_files = [
            ("config.txt", blueos_startup_update.config_file),
            ("cmdline.txt", blueos_startup_update.cmdline_file),
        ]
