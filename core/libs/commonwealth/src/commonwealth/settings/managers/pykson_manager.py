...
class PyksonManager:
    ...
    def __init__(
        self,
        project_name: str,
        settings_type: Type[PyksonSettings],
        config_folder: Optional[pathlib.Path] = None,
        load: bool = True,
    ) -> None:
        assert project_name, "project_name should be not empty"
        assert issubclass(settings_type, PyksonSettings), "settings_type should use PyksonSettings as subclass"
    ...
    def settings(self) -> Any:
        ...
        Returns:
            [Type[PyksonSettings]]: The settings defined in the constructor
        """
    ...
    def settings(self, value: Any) -> None:
        ...
        Args:
            value ([Type[PyksonSettings]]): The settings defined in the constructor
        """
        if not self._settings:
            self.load()
    ...
    def settings_file_path(self) -> pathlib.Path:
        ...
        """
        return self.config_folder.joinpath(f"{PyksonManager.SETTINGS_NAME_PREFIX}{self.settings_type.VERSION}.json")

    @staticmethod
    def load_from_file(settings_type: Type[PyksonSettings], file_path: pathlib.Path) -> Any:
        """Load settings from a generic location and settings type

        Args:
            settings_type (PyksonSettings): Settings type that inherits from PyksonSettings.
