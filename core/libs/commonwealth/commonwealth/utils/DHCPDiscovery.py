...
async def discover_dhcp_servers(iface: str, timeout: int = 15) -> List[str]:
    ...
    try:
        ...
        if process.returncode != 0:
            print(f"nmap output: {output}")
            print(f"nmap error: {error}")
            raise DHCPDiscoveryError(f"nmap failed: {stderr.decode()}")
...
async def has_dhcp_server(interface: str, timeout: int = 15) -> bool:
    """
    Check if any DHCP server is present on the network
    ...
    Returns:
        True if at least one DHCP server is found, False otherwise
    """
    try:
        servers = await discover_dhcp_servers(interface, timeout)
        logger.info(f"Found {len(servers)} DHCP server(s) on {interface}")
        logger.info(f"Servers: {servers}")
        return len(servers) > 0
    except DHCPDiscoveryError as e:
        logger.error(f"Failed to discover DHCP servers on {interface}. {e}")
